...
export CURRENT_VERSION:=$(shell python3.6 -c "import os; about={}; exec(open(os.path.join('batu', '_meta.py')).read(), about); print(about['__version__'])")
...
uninstall: ## uninstall the package
	pip uninstall -y $(PIP_ARGS) batu || true
...
env-create: init-hook ## (re)create a development environment using tox
	tox -e batu --recreate
	@echo -e "\r\nYou can activate the environment with:\r\n\r\n$$ source ./.tox/batu/bin/activate\r\n"
	./.tox/batu/bin/pip install pip==21.2.4
...
env-upgrade-package: ## upgrade the package specified on PACKAGE
	@[ "${PACKAGE}" ] || ( echo "PACKAGE is not set"; exit 1 )
	pip-compile --upgrade-package ${PACKAGE} --no-emit-index-url --no-header --no-emit-trusted-host --output-file constraints.txt requirements.in requirements-dev.in
	pip-compile --no-emit-index-url --no-header --no-emit-trusted-host --output-file requirements.txt requirements.in constraints.in

env-upgrade-dev-package: ## upgrade the development package specified on PACKAGE
	@[ "${PACKAGE}" ] || ( echo "PACKAGE is not set"; exit 1 )
	pip-compile --upgrade-package ${PACKAGE} --no-emit-index-url --no-header --no-emit-trusted-host --output-file constraints.txt requirements.in requirements-dev.in
	pip-compile --no-emit-index-url --no-header --no-emit-trusted-host --output-file requirements-dev.txt requirements-dev.in constraints.in

env-upgrade-all: ## upgrade all dependencies
	pip-compile --upgrade --no-emit-index-url --no-header --no-emit-trusted-host --output-file constraints.txt requirements.in requirements-dev.in
	pip-compile --no-emit-index-url --no-header --no-emit-trusted-host --output-file requirements.txt requirements.in constraints.in
	pip-compile --no-emit-index-url --no-header --no-emit-trusted-host --output-file requirements-dev.txt requirements-dev.in constraints.in
...
lint: ## static code analysis with pylint
	pylint --rcfile batu/.pylintrc --disable=$(EXTRA_DISABLE) -j 0 batu
...
coverage: ## check test coverage
	coverage run --source batu -m pytest
...
docs: ## generate project docs
	rm -f docs/batu.rst
	find docs -name 'batu.*.rst' -exec rm -f {} +
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ batu
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
doc-test-plan: dir-health
	...
	@sed -i -e 's/project_name/batu/g' docs/health/test_plan.rst
...
doc-release: dir-operations
	...
	@sed -i -e 's/project_name/batu/g' docs/operations/release.rst
	@echo Release created!

doc-run: dir-operations
	@cp -n ~/projects/daskeleton/templates/operations/run.rst docs/operations/run.rst
	@sed -i -e 's/project_name/batu/g' docs/operations/run.rst
	@echo Run created!
...
doc-usage: dir-support
	...
	@sed -i -e 's/project_name/batu/g' docs/support/usage.rst
	@sed -i -e 's/project_name/batu/g' docs/support/usage.rst
	@echo Usage created!
...
publish-dev: ## publish packages to the repository.
	...
	twine upload --repository dasnano dist/batu*
...
docker-run: ## run the specified command on docker (defaults to /bin/bash)
	docker-compose -f docker/dev/docker-compose.yml run --rm -u $(CURRENT_USER) batu $(MAKE) $(TARGET)

docker-shell: ## drop the user into a shell inside a docker container
	docker-compose -f docker/dev/docker-compose.yml run --rm -u $(CURRENT_USER) batu
...
ci-dist-dev: ## build the package (for development) for every supported version on CI (GitLab)
	tox -e batu -- $(MAKE) dist-dev

ci-publish: ## publish built packages to the appropriate repository on CI (GitLab)
	tox -e batu -- $(MAKE) publish ## publish the packages generated with dist or dist-dev on CI (GitLab)
...
jetson-configure: git-configure nexus-configure
	...
	ssh -p $${PORT:=22} $(DEVICE) "pip3 install pip-tools==6.4.0"
...
dasgate-admin-configure:
	@[ "${DEVICE}" ] || ( echo "DEVICE is not set"; exit 1 )
	@echo "Generating a new ssh key pair on device's user space..."
	sudo apt-get install -y xclip
	ssh -p $${PORT:=22} $(DEVICE) "ssh-keygen -f ~/.ssh/gitlab_id_rsa -q -N \"\" <<< y >/dev/null && cat ~/.ssh/gitlab_id_rsa.pub" | xclip -sel clip
	@echo "A new public key of target device has been copied to your clipboard. Paste it in GitLab > Settings > SSH Keys"
	firefox https://gitlab.com/-/profile/keys &
	@echo "Coping private key from device's user space to dasgate_admin..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo cp ~/.ssh/gitlab_id_rsa /home/dasgate_admin/.ssh/"
	ssh -p $${PORT:=22} $(DEVICE) "sudo cp ~/.ssh/gitlab_id_rsa.pub /home/dasgate_admin/.ssh/"
	@echo "Applying proper permissions to private key on device's dasgate_admin space..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo chown dasgate_admin:dasgate_admin /home/dasgate_admin/.ssh/gitlab_id_rsa"
	@echo "Cleaning SSH key pair from device's user space..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo rm ~/.ssh/gitlab_id_rsa"
	ssh -p $${PORT:=22} $(DEVICE) "sudo rm ~/.ssh/gitlab_id_rsa.pub"
	@echo "Coping pypi configuration from host to device's user space..."
	scp -P $${PORT:=22} ~/.pypirc $(DEVICE):~/.pypirc
	@echo "Coping pypi configuration from device's user space to dasgate_admin..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo cp ~/.pypirc /home/dasgate_admin/.pypirc"
	@echo "Cleaning pypi configuration from device's user space..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo rm ~/.pypirc"
	@echo "Applying proper permissions to pypi configuration on device's dasgate_admin space..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo chown dasgate_admin:dasgate_admin /home/dasgate_admin/.pypirc"
	@echo "Configuring Nexus credentials on device's user space..."
	ssh -p $${PORT:=22} $(DEVICE) "echo 'export NEXUS_DASNANO_USER=$(NEXUS_DASNANO_USER)' >> ~/.nexus_cred"
	@ssh -p $${PORT:=22} $(DEVICE) "echo 'export NEXUS_DASNANO_PASSWORD=$(NEXUS_DASNANO_PASSWORD)' >> ~/.nexus_cred"
	@echo "Copying Nexus credentials to device's dasgate_admin space..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo cp ~/.nexus_cred /home/dasgate_admin/.nexus_cred"
	@echo "Cleaning Nexus credentials from device's user space..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo rm ~/.nexus_cred"
	@echo "Applying proper permissions to Nexus credentials on device's dasgate_admin space..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo chown dasgate_admin:dasgate_admin /home/dasgate_admin/.nexus_cred"
	@echo "Relaxing bashrc permissions on device's dasgate_admin space..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo chmod 646 /home/dasgate_admin/.bashrc"
	@echo "Configuring bashrc to load Nexus credentials on device's dasgate_admin space..."
	ssh -p $${PORT:=22} $(DEVICE) "echo 'source /home/dasgate_admin/.nexus_cred' >> /home/dasgate_admin/.bashrc"
	ssh -p $${PORT:=22} $(DEVICE) "echo 'export PATH=/home/dasgate_admin/.local/bin:\$$PATH' >> /home/dasgate_admin/.bashrc"
	@echo "Installing required packages to make releases on device's dasgate_admin space..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo runuser -u dasgate_admin -- pip3 install --user pip-tools==6.4.0"
	ssh -p $${PORT:=22} $(DEVICE) "sudo runuser -u dasgate_admin -- pip3 install --user cython"
	@echo "Constrain bashrc permissions on device's dasgate_admin space..."
	ssh -p $${PORT:=22} $(DEVICE) "sudo chmod 644 /home/dasgate_admin/.bashrc"

dasgate-admin-clean:
	ssh -p $${PORT:=22} $(DEVICE) "sudo rm /home/dasgate_admin/.ssh/gitlab_id_rsa"
	ssh -p $${PORT:=22} $(DEVICE) "sudo rm /home/dasgate_admin/.ssh/gitlab_id_rsa.pub"
	ssh -p $${PORT:=22} $(DEVICE) "sudo rm /home/dasgate_admin/.pypirc"
	ssh -p $${PORT:=22} $(DEVICE) "sudo rm /home/dasgate_admin/.nexus_cred"
	ssh -p $${PORT:=22} $(DEVICE) "sudo runuser -u dasgate_admin -- pip3 uninstall pip-tools==6.4.0"
	ssh -p $${PORT:=22} $(DEVICE) "sudo runuser -u dasgate_admin -- pip3 uninstall cython"
...
vpn-get-ip: pull-daskeleton ## query terminal ip to vpn
	...
	@[ "${DEV}" ] || ( echo "DEV is not set"; exit 2 )
	@[ "${HUB}" ] || ( echo "HUB is not set"; exit 3 )
	python3 ~/projects/daskeleton/scripts/query_vpn.py get-ip --environment=${ENV} --hubname=${HUB} --terminal=${DEV}

vpn-get-sessions: pull-daskeleton ## query terminal ip to vpn
	@[ "${ENV}" ] || ( echo "ENV is not set"; exit 1 )
	@[ "${DEV}" ] || ( echo "DEV is not set"; exit 2 )
	python3 ~/projects/daskeleton/scripts/query_vpn.py get-sessions --environment=${ENV} --hubname=${HUB}

vpn-configure: pull-daskeleton enable-sudo ## connect to terminal through vpn
	@[ "${ENV}" ] || ( echo "ENV is not set"; exit 1 )
	@[ "${HUB}" ] || ( echo "HUB is not set"; exit 2 )
	~/projects/daskeleton/scripts/configure_vpn.sh ENVIRONMENT=${ENV} HUB_NAME=${HUB}

vpn-connect: vpn-configure ## connect to terminal through vpn
	@[ "${DEV}" ] || ( echo "DEV is not set"; exit 1 )
	ssh -p $${PORT:=22} $(DEV)

vpn-flash-connect: vpn-configure ## query terminal ip to vpn
	@[ "${ENV}" ] || ( echo "ENV is not set"; exit 1 )
	@[ "${DEV}" ] || ( echo "DEV is not set"; exit 2 )
	@[ "${HUB}" ] || ( echo "HUB is not set"; exit 3 )
	python3 ~/projects/daskeleton/scripts/query_vpn.py get-ip --environment=${ENV} --hubname=${HUB} --terminal=${DEV} > /tmp/ip_info.txt
	terminal_ip=$(shell cat "/tmp/ip_info.txt" | awk -F ":   " '/VPN IP:  /{printf $$2}');\
	ssh -p $${PORT:=22} $${terminal_ip}
